...
void PWM_Recv_Init() {
  ...
  //
  P2DIR  &= ~PWM_RECV_PIN;
  P2SEL  |=  PWM_RECV_PIN; // Select pin 2.0 as input interrupt
  P2SEL2 &= ~PWM_RECV_PIN; // Select pin 2.0 as input interrupt
}
        ...
        case RISING_EDGE:
            if (first) { // Offset to the first rising edge
                ...
                if (++samples >= MAX_SAMPLE) { // 8 samples obtained
                    ...
                    tmp1 >>= AVERAGE; // Calculate the average (/ 2^3)
                    tmp2 >>= AVERAGE;
